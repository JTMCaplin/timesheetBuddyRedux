define(["lib/react", "LoginPanel", "TimelinePanel", "StatCalculator"], function(
  React,
  LoginPanel,
  TimelinePanel,
  StatCalculator
  ...
  var JiraStatisticsCompilerPanel = React.createClass({
    render: function() {
      return React.DOM.div(
        ...
        TimelinePanel({ jiraId: this.props.jiraId, events: this.props.events })
      );
    },
    ...
    selectProject: function(projectIds, user) {
      getDataWithJSON(
        function(response) {
          ...
          window.last10days = Array.from({ length: 10 }, (v, k) => k).map(
            function(v) {
              var date = new Date();
          ...
          console.log(
            "Total issues worked on in last 10 days:",
            data.issues.length
          );
          console.log(
            "Issues you have worked on in last 10 days:",
            yourData.length
          );
